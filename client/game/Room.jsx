import React from "react"

import Student from "./Student.jsx"

import { TimeSync } from "meteor/mizzao:timesync"
import moment from "moment"

export default class Room extends React.Component {
  state = { hovered: false }

  handleDragOver = (e) => {
    console.log("timeSync", moment(TimeSync.serverTime(null, 1000)))
    e.preventDefault()
    e.dataTransfer.dropEffect = "move"
    this.setState({ hovered: true })
  }

  handleDragLeave = (e) => {
    this.setState({ hovered: false })
  }

  handleDrop = (e) => {
    const { stage, player, room } = this.props
    const groupTag = player.get("groupIdTag")

    console.log(e.dataTransfer.getData("text/plain"))
    const student = e.dataTransfer.getData("text/plain")
    console.log(student.split(" ").length)
    const paused = stage.get("paused")

    if (student.split(" ").length === 1) {
      const isSolo = stage.name.split("_")[0] === "Independent"
      // {
      //   isSolo
      //     ? player.stage.set(`student-${student}-dragger`, null)
      //     : stage.set(`${groupTag}-student-${student}-dragger`, null)
      // } //maybe this fixes the problem of stucked colors
      const currentRoom = isSolo
        ? player.stage.get(`student-${student}-room`)
        : stage.get(`${groupTag}-student-${student}-room`)

      this.setState({ hovered: false })

      // Avoid any unwanted drops!
      // We're using the native DnD system, which mean people can drag anything
      // onto these drop zones (e.g. files from their desktop) so we check this
      // is an existing student first.
      if (currentRoom === room) {
        return
      }

      if (isSolo) {
        player.stage.set(`student-${student}-room`, room)

        player.stage.append("log", {
          verb: "movedStudent",
          subjectId: player._id,
          object: student,
          target: room,
          // at: new Date()
          at: moment(TimeSync.serverTime(null, 1000)),
        })
      } else {
        stage.set(`${groupTag}-student-${student}-room`, room)

        if (player.get("role") === "Leader" && !paused) {
          stage.append(`${groupTag}-log`, {
            verb: "movedStudent",
            subjectId: player._id,
            object: student,
            target: room,
            // at: new Date()
            at: moment(TimeSync.serverTime(null, 1000)),
          })
        }
      }

      // console.log("room moment", moment(TimeSync.serverTime(null, 1000)))
    }
  }

  render() {
    const { room, isDeck, stage, player, game } = this.props
    const { hovered } = this.state
    const isSolo = stage.name.split("_")[0] === "Independent"
    const students = []
    const groupTag = player.get("groupIdTag")
    const task = stage.get("task")
    task.students.forEach((student) => {
      if (isSolo) {
        if (player.stage.get(`student-${student}-room`) === room) {
          students.push(student)
        }
      } else {
        if (stage.get(`${groupTag}-student-${student}-room`) === room) {
          students.push(student)
        }
      }
    })
    // console.log(task.students) //['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I']
    // console.log(room, students)

    const classNameRoom = isDeck ? "deck bp3-elevation-1" : "room"
    const classNameHovered = hovered ? "bp3-elevation-3" : ""
    return (
      <div
        onDrop={this.handleDrop}
        onDragOver={this.handleDragOver}
        onDragLeave={this.handleDragLeave}
        className={`bp3-card ${classNameRoom} ${classNameHovered}`}
      >
        {isDeck ? null : <h6 className="bp3-heading">Room {room}</h6>}
        {students.map((student) => {
          return (
            <Student
              onDragStart={this.handleDragStart}
              key={isSolo ? student + "_" + player.id : student}
              student={student}
              room={room}
              isSolo={isSolo}
              {...this.props}
            />
          )
        })}
      </div>
    )
  }
}
